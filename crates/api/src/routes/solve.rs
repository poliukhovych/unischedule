...
#[derive(Deserialize)]
pub struct SolveIn { #[serde(flatten)] pub env: SolveEnvelope }

#[derive(serde::Serialize, ToSchema)]
pub struct JobCreated { pub jobId: String, pub status: &'static str }
...
pub async fn reoptimize(State(state): State<AppState>, Json(env): Json<SolveEnvelope>) -> Json<JobCreated> {
    let id = state.jobs.enqueue(env);
    Json(JobCreated { jobId: id.0, status: "queued" })
}
